...
function merge(kind, flag, describable) {
    return Object.assign({ kind }, flag, { description: describable.description, longDescription: describable.longDescription });
}
function option(kind, options, parse) {
    return merge(kind, command_1.flags.build(Object.assign(options, { parse }))(), options);
}
// oclif
function buildBoolean(options) {
    return merge('boolean', command_1.flags.boolean(options), options);
}
function buildEnum(options) {
    return Object.assign({ kind: 'enum', type: 'option' }, command_1.flags.enum(options), { options: options.options, description: options.description, longDescription: options.longDescription });
}
...
function buildArray(options) {
    ...
    if ('map' in options) {
        const { options: values } = options, rest = __rest(options, ["options"]);
        const allowed = new Set(values);
        return option(kind, rest, (val) => {
            const vals = val.split(options.delimiter || ',').map(options.map);
            validateValue(allowed.size === 0 || vals.every(t => allowed.has(t)), val, kind, ` ${messages.getMessage('FormattingMessageArrayOption', [Array.from(allowed).toString()])}`);
            return vals;
        });
    }
    else {
...
function buildBuiltin(options = {}) {
    return Object.assign({}, options, { type: 'builtin' });
}
exports.flags = {
    // oclif
    /**
